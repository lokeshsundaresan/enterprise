...
public class RemoveProduct extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		HA dao=new HA();
		int result=dao.removeProduct(model);
		
		  if(result > 0 )
	 	   {
	 		   request.setAttribute("status", "Product successfully removed");
	 		   request.getRequestDispatcher("ModifyProduct").forward(request, response);
	 	   }
	 	   else
	 	   {
